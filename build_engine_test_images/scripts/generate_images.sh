...
function build_upgrade_test_images() {
  # 1 arch
  # 2 upgrade_image

  git clone https://github.com/longhorn/longhorn-engine.git
  sed -i "s/.*ARG DAPPER_HOST_ARCH=.*/ARG DAPPER_HOST_ARCH=\$1/" longhorn-engine/Dockerfile.dapper
  cd longhorn-engine
...
build_upgrade_test_images ${arch} ${upgrade_image}
